        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for val_data in tqdm(val_loader): 
                    ...
                    if opt['task'] == 'MCSR':            
                        sr_img_1 = visuals["im1_restore"][0, 0]  # (1, w, h)
                        gt_img_1 = visuals["im1_GT"][0, 0]  # (1, w, h)                                     
                        # calculate PSNR
                        cur_psnr_im1 = util.calculate_psnr(sr_img_1.numpy()*255., gt_img_1.numpy()*255.)
                        # print(i, cur_psnr_im1)
                        avg_psnr_im1 += cur_psnr_im1
                        idx += 1   
                    
                    else:
                        img_num = visuals["im1_GT"].shape[1]
                        for i in range(img_num):
                            sr_img_1 = visuals["im1_restore"][0, i, 0, :, :]  # (1, w, h)
                            gt_img_1 = visuals["im1_GT"][0, i, 0, :, :]  # (1, w, h)                                     
                            # calculate PSNR
                            cur_psnr_im1 = util.calculate_psnr(sr_img_1.numpy()*255., gt_img_1.numpy()*255.)
                            # print(i, cur_psnr_im1)
                            avg_psnr_im1 += cur_psnr_im1
